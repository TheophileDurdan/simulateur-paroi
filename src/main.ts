...
import { defaultLayers } from "./materials";
...
let layers: Layer[] = defaultLayers();
...
let facade: FacadeOrientation = { ...DEFAULT_FACADE };
...
function currentQSolar(): number {
  return SOLAR_ABSORPTIVITY * (currentSolarPercent() / 100) * G_MAX;
}
...
function frame(now: number) {
  ...
  panel.update({
    ...
    tAirInt: solver.tAirInt,
    tExt: currentTExt(),
    solarPercent: currentSolarPercent(),
    extAuto,
  });
